...
이전에는 [WebFlux에서 Functional Endpoint 선택하기](/ko/posts/spring-webflux-router/)라는 글을 쓴 적이 있습니다. 이번에는 `Controller`/`Service`와 `Router`/`Handler`를 비교하려는 것이 아니라, `Functional Endpoint`를 쓸 때 실제로 어떤 구조로 구현하는 게 좋은지에 대해 생각한 내용을 정리해 보려 합니다.
