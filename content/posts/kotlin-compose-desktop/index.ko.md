...
[SwiftUI를 써 본 첫인상 2](/ko/posts/swift-ui-first-impression-2/)에서도 접한 상태 관리입니다만, Compose에서도 마찬가지로 GUI를 취급하게 되므로, 상태 관리가 중요합니다. 이번에는 앱으로서의 화면이 하나 밖에 없기 때문에, 복수의 화면에 걸쳐 상태를 관리할 필요는 없을까라고 생각했습니다만, 그래도 역시 처리를 실시하기 위해서는 앱 전체에서 공유하는 상태로서 관리가 필요한 것이 몇가지 있었습니다.
