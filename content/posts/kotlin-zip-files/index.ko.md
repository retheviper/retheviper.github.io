...
NIO에 관해서는 이전의 포스트로 몇번인가 언급한 적이 있습니다만, 서버와 같이 빈번하고 동시 실행수가 많은 경우는 적극적으로 채용하는 것이 좋다고 생각합니다. 또, Java 1.8 이후부터 추가된 메소드로 꽤 편리하게 사용할 수 있는 기능이 많기 때문에, IO를 NIO로 바꾸는 것만으로 코드의 양을 꽤 줄일 수 있을 가능성도 있습니다.
특히 디렉토리를 지정했을 경우, 그 디렉토리의 자식 요소를 순환하기 위해서는 NIO의 `Files`이 제공하는 기능이 강력하므로, 이번은 그것을 적극 활용하기로 합니다.
### 서명 변경
...
이렇게 하면, 메소드의 외측(호출원)에서의 사용법은 보다 간단하게 되고, 압축원의 데이터를 읽어들일 때에 사용하는`InputStream`은 안으로 닫고 있는데 인수의`OutputStream`은 외부에서 닫는다고 하는 복잡한 상황은 피할 수 있습니다.
### 재귀를 없애기
