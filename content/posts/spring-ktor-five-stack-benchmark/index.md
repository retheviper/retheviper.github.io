...
### 1. 小さいJSONは混戦で、大きいJSONではSpring MVCが強い

`smallJson`は、いまは一つのスタックがはっきり抜けているというより上位がかなり詰まっています。同時実行128ではKtor R2DBCが27,979 req/s、Ktor JDBCが27,790 req/s、Spring WebFlux R2DBCが27,737 req/sでほぼ横並びでしたし、同時実行256でもSpring MVC platformが26,531 req/sで先頭だったものの、Spring WebFlux R2DBC 26,309 req/s、Ktor R2DBC 25,047 req/sも十分近い数字でした。DBアクセスがないぶんフレームワークとシリアライズ経路の差が出やすいパスですが、上位陣の差は以前よりかなり圧縮されています。

しかし`largeJson`になると話が変わります。同時実行256ではSpring MVC platformが3,273 req/s、Spring MVC virtualが2,822 req/sで、Ktor JDBCは2,376 req/s、Spring WebFlux R2DBCは2,174 req/s、Ktor R2DBCは877 req/sでした。ここは以前から言われていた通り、`kotlinx.serialization`が大きいJSONではまだJacksonより不利な面を残しているように見えますし、特にKtor R2DBCはここで差がかなり大きく出ました。
...
仮想スレッドも引き続き面白いのですが、今回はDB readでもplatformとvirtualがシナリオごとに勝ち負けを分けました。`bookById`と`bookSearch`はplatformが勝ち、`bookList`と`bookList500`はvirtualが勝っています。`aggregateReport`も同時実行256でSpring MVC virtual 1,020 req/s、platform 948 req/s、Ktor JDBC 917 req/sとかなり接戦でした。仮想スレッドは確かに有力ですが、入れた瞬間に何でも速くなる魔法ではない、というのはやはり変わりません。

### 3. Ktor JDBCは悪くないが、Ktor R2DBCはかなり苦しい

Ktor側だけを見ると、JDBC版とR2DBC版の差は今回もかなりはっきり出ました。例えば同時実行256で、`bookList500`はKtor JDBCが1,388 req/s、Ktor R2DBCが466 req/s、`largeJson`はKtor JDBCが2,376 req/s、Ktor R2DBCが877 req/sでした。`checkoutCreate`ではKtor JDBCが2,959 req/s、Ktor R2DBCが2,941 req/sでほぼ並びましたが、少なくとも「R2DBCにすると全体的に有利になる」という結果ではありませんでした。
...
数字だけ見ると同時実行256でKtor JDBCが3,248 req/sと最も高いですが、失敗数も9,816あります。Spring WebFluxは2,312 req/sで失敗6,995、Spring MVC platformは2,093 req/sで失敗6,327、Ktor R2DBCは1,375 req/sで失敗4,177でした。つまり、ここは「速い」の中に「速く失敗している」がかなり含まれているので、通常のシナリオとは少し別物として扱うべきですね。

### 5. aggregateReportは思ったより差がつかなかった

`aggregateReport`は、今回は特定の一つが大きく抜けるというより上位がかなり密集していました。同時実行128ではKtor JDBCが984 req/sで先頭でしたが、Spring WebFlux R2DBC 973、Spring MVC virtual 971、Spring MVC platform 962がすぐ後ろにつけています。同時実行256ではSpring MVC virtualが1,020 req/sで先頭、Spring MVC platformが948、Ktor JDBCが917、Spring WebFlux R2DBCが893でした。このシナリオは「例外的にKtor JDBCが圧勝する」というより、各実装がかなり近い位置に並んだと見るほうが実態に近いと思います。
...
R2DBCについても、少なくともこの条件では「JDBCを明確に置き換えるほど速い」とは言いづらい結果でした。思想としては綺麗でも、DBアクセス込みの実アプリでは別のオーバーヘッドやロックの振る舞いまで含めて評価する必要がある、という印象です。
...
その意味では、性能重視であれば今でもSpring MVCのようなオーソドックスな構成はかなり有力です。一方で、コード量や読みやすさ、Kotlinらしさを優先するならKtorを選ぶのも十分ありだと思います。実際、Ktor JDBCはread系や一部write系で十分戦えており、アプリケーションの性質次第では十分実用的でしょう。
