...
일반적인 Java 프로젝트라면 외부 설정 파일(YAML)을 작성해서 그 값을 읽어들이는 것은 [외부 설정 파일 읽는 클래스 만들기](/ko/posts/java-yaml-for-configuration/)처럼 할 수 있습니다. 그런데 이번에는 Spring 프로젝트에서 비슷한 일을 하게 되었습니다. Spring은 YAML을 읽을 때만의 방식과 규칙이 있고, 읽어들인 값은 Bean으로 만들 수 있어서 애플리케이션 안 어디서든 `@Autowired`로 꺼내 쓸 수 있다는 장점이 있습니다.
